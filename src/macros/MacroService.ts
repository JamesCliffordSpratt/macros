...
async calculateMacrosFromLinesAsync(
	...
	this.plugin.logger.debug(`Original lines: ${lines.length}, Merged lines: ${mergedLines.length}`);
...
processNutritionalDataFromLines(ids: string[]): {
	...
	this.plugin.logger.debug(`MacroService processing nutrition data for ${ids.length} IDs: ${ids.join(', ')}`);
	...
	for (const id of ids) {
		...
		this.plugin.logger.debug(`MacroService after merging: ${mergedLines.length} lines for ID: ${id}`);
