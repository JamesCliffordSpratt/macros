...
export class APIService {
	...
	getActiveApiKey(): string {
		const key = this.plugin.settings.fatSecretApiKey?.trim();
		if (!key) {
			throw new Error('API key not configured. Please add your FatSecret API credentials in settings.');
		}
		return key;
	}
	...
	getActiveApiSecret(): string {
		const secret = this.plugin.settings.fatSecretApiSecret?.trim();
		if (!secret) {
			throw new Error('API secret not configured. Please add your FatSecret API credentials in settings.');
		}
		return secret;
	}
