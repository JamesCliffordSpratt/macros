...
export function registerProcessors(plugin: MacrosPlugin): void {
	...
	plugin.registerMarkdownCodeBlockProcessor('macros', async (source: string, el: HTMLElement) => {
		...
		const renderTableFromLines = (lines: string[]) => {
			...
			function processItem(foodQuery: string, specifiedQuantity?: number | null): MacroRow | null {
			...
			if (otherGroup.rows.length > 0) groups.push(otherGroup);
			...
			const removeMacroLine = async (macroLine: string) => {
				const activeFile = plugin.app.workspace.getActiveFile();
				...
				try {
					...
					await refreshTable();
					await plugin.redrawAllMacrospc();
					await plugin.redrawAllMacrocalc();
				} catch (error) {
	...
	plugin.registerMarkdownCodeBlockProcessor('macrospc', async (source: string, el: HTMLElement) => {
		...
		for (const id of ids) {
			if (!plugin.macroTables.has(id)) {
				const loaded = await plugin.loadMacroTableFromVault(id);
				if (loaded) {
	 ...
	 */
	plugin.registerMarkdownCodeBlockProcessor('macroscalc', async (source: string, el: HTMLElement) => {
		let lines = source.split("\n").map(l => l.trim()).filter(l => l !== '');
		if (lines.length === 0) {
		...
		for (const id of ids) {
			if (!plugin.macroTables.has(id)) {
				const tableLines = await plugin.loadMacroTableFromVault(id);
				if (tableLines) {
		...
		const breakdown: { id: string, totals: { calories: number, protein: number, fat: number, carbs: number } }[] = [];
		
		function processFoodItem(foodQuery: string, specifiedQuantity: number | null): NutritionData {
			const matchingFile = findMatchingFoodFile(plugin.getFilesInFolder(plugin.settings.storageFolder), foodQuery);
			if (!matchingFile) return { calories: 0, protein: 0, fat: 0, carbs: 0 };
		...
		for (const id of ids) {
			...
			for (const line of tableLines) {
				if (line.toLowerCase().startsWith("meal:")) {
					...
					const meal = plugin.settings.mealTemplates.find(m =>
